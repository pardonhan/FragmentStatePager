 ...
 * 按照一定顺序进行答题
 * @author hanfl
 * @version [v1.0, 2015-08-01]
 * @see [BaseActivity]
 * @since [v1.0]
 */

public class OrdinalExamActivity extends BaseActivity {
	...
	private Button btn_submit;// 答案提交按钮
	...
	private void initView() {
		popDialog = new PopupWindowDialog(OrdinalExamActivity.this, null);
	...
	private void initPopupWindow() {
		...
		title_re.getBackground().setAlpha(100);
		...
		pwMyPopWindow.setAnimationStyle(R.style.AnimationFade);
		egadapter = new ExamGridAdapter(OrdinalExamActivity.this);
	...
	private void getExamData() {
		if (m_Dialog != null) {
			...
			m_Dialog = MsgUtil.ShowLoadDialog(OrdinalExamActivity.this, "请稍后",
					"正在加载数据");
		}
		new Thread() {
			@Override
			public void run() {
				...
				_List = DataBaseUtils.getOrdinalQuestionInfo(OrdinalExamActivity.this);
				if(_List.size()>0){
	...
	private void saveExamResult(final QuestionInfo questionInfo){
		new Thread(){
			public void run() {
				DataBaseUtils.saveExamResult(OrdinalExamActivity.this, questionInfo,"JXD7_EX_ORDINALRESULT", "1");
			}
		}.start();
	}
	...
	private void showExam() {
		...
		if (asize > 0) {
			...
			if (qf.typeid.equals("1")) {
				...
				img_practise
						.setImageResource(R.drawable.practise_danxuanti_day);
			...
			if (qf.typeid.equals("2")) {
				...
				img_practise
						.setImageResource(R.drawable.practise_duoxuanti_day);
				if (qf.selectAnswer.size() != 0 && qf.wrongModel != 3) {
					checkMcqAnswer();
				}
				if (qf.selectAnswer.size() != 0 && qf.wrongModel == 3) {
	...
	OnClickListener listener = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.submitAs:
				 ...
				 * MsgUtil.ShowErrMsg("请至少选择两个答案", OrdinalExamActivity.this);
				 * break; }
				 */
				// setClickable(false)
				for (int i = 0; i < li_size; i++) {
				...
				if (PublicUtils.isSetEqual(qfo.selectAnswer, qfo.answer)) {
					...
					for (String mc : qfo.answer) {
						for (int i = 0; i < asSize; i++) {
							if (ci_TextView.get(i).getText().toString()
									.equals(mc)) {
			...
			case R.id.btn_next:
				...
				if (current < count - 1) {
					...
					if (asize > 0) {
						...
						btn_submit.setVisibility(View.GONE);
						img_practise
								.setImageResource(R.drawable.practise_panduanti_day);
						for (int i = 0; i < 2; i++) {
							li_LinearLayout.get(i).setVisibility(View.VISIBLE);
							li_LinearLayout.get(i).setClickable(true);
							li_LinearLayout.get(i).setOnClickListener(listener);
					...
					/*new AlertDialog.Builder(OrdinalExamActivity.this)
							.setTitle("提示")
							...
							.setPositiveButton("确定",
									new DialogInterface.OnClickListener() {
										...
										public void onClick(
												...
												int which) {
											OrdinalExamActivity.this.finish();
										}
			...
			case R.id.btn_previous:
				// previoue question
				if (current > 0) {
					tv_questionNum.setText((current) + "/" + count);// top right
																	// corner
																	// show the
																	// question
																	// number
	...
	private void checkAnswer() {
		...
		if (PublicUtils.isSetEqual(qf.selectAnswer, qf.answer)) {
			for (int i = 0; i < li_size; i++) {
				if (qf.selectAnswer.contains(ci_TextView.get(i).getText()
						.toString())) {
		...
		case 1:
			...
			if (PublicUtils.isSetEqual(qf.answer, qf.selectAnswer)) {
				...
				for (int j = 0; j < answerSize; j++) {
					...
					if (qf.answer.contains(ci_TextView.get(j).getText()
							.toString())) {
						DebugLog.i("--single choice--show the ture answer--"
								+ ci_TextView.get(j).getText().toString());
						ci_TextView.get(j).setTextColor(Color.RED);
						tv_TextView.get(j).setTextColor(Color.GREEN);
					}
					if (qf.selectAnswer.contains(ci_TextView.get(j).getText()
							.toString())) {
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 1) {
				...
				if(ci.getOrdinalExam(sp)!=0){
					current =ci.getOrdinalExam(sp);
					showExam();
				}else{
					QuestionInfo qf = mList.get(current);
					ArrayList<AnswerInfo> af = (ArrayList<AnswerInfo>) qf.answerItem;
					...
					if (asize > 0) {
						for (int i = 0; i < asize; i++) {
							li_LinearLayout.get(i).setVisibility(View.VISIBLE);
							tv_TextView.get(i).setText(
									af.get(i).itemcontent.toString());
						}
						if (qf.typeid.equals("1")) {
							// single choice
							btn_submit.setVisibility(View.GONE);
			...
			if (msg.what == 2) {
				...
				Intent intent = new Intent(OrdinalExamActivity.this,MainActivity.class);
