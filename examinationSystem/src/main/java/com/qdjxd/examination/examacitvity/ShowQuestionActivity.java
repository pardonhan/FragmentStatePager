...
public class ShowQuestionActivity extends BaseActivity {
	private QuestionInfo qf;
	private List<AnswerInfo> anList = new ArrayList<AnswerInfo>();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		qf = (QuestionInfo) intent.getSerializableExtra("question");
		anList = qf.answerItem;
		initView();
		...
		if(qf.typeid.equals("3")){
			...
			for(int i=0;i<anList.size();i++){
				li_LinearLayout.get(i).setVisibility(View.VISIBLE);
				tv_TextView.get(i).setText(anList.get(i).itemcontent.toString());
